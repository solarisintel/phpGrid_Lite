...
class C_DataGrid
{
	...
	$_82AE25C96B23C3058D7199BA4BFA2713 = '', $extra_params = '')
	{
		...
		if (DB_TYPE == 'mysqli') {
		   $this->db->executeQuery("set names '".CharSetNames."'");
		}
		$this->_0809869D6A1EEF2DEC1395F670A46E20 = $this->db->GetRecordCount(
		$this->db->executeQuery($sql));
		$results = $this->db->selectLimit($sql, 1, 1);
	...
	public function _467180D7C72C5DCE3F4AFED765F798AF ()
	{
		...
		$this->_0809869D6A1EEF2DEC1395F670A46E20 = $_BA14039F0E958B2349059AB1B041943B->GetRecordCount(
		$_BA14039F0E958B2349059AB1B041943B->executeQuery(
		$this->sql));
		$results = $_BA14039F0E958B2349059AB1B041943B->selectLimit(
		$this->sql, 1, 1);
	...
	public function set_colModel ($results)
	{
		...
		for ($i = 0; $i < $this->fieldCount; $i ++)
		{
			...
			$_3F200CC61D0ED71A411120D72C51A4AD = $_BA14039F0E958B2349059AB1B041943B->meta_type($results, $i);
			$cols = array();
			$cols['name'] = $fieldName;
