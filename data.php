...
if ($_760EB019CDB8F62FF8AA9C0469129954) {
	...
	foreach ( $_REQUEST as $key => $value ) {
		if (in_array ( $key, $_67BACCE5E5580D4DF8F07E46F4F129B9 )) {
			$_F263E5B84D6C79F0B108E8DD213FE515 = $db->_39ADD6B4099DB99B597E2C89CADDDFB3 ( $rs, $db->_4CF52E24237F544BE9BC663B4DF0F9D7 ( $rs, $key ) );
			switch ($_F263E5B84D6C79F0B108E8DD213FE515) {
				case 'I' :
				case 'N' :
				case 'R' :
				case 'L' :
					$_7B6C2BDBBD2AF03C64A520410964D2F2 .= " AND " . $key . " = " . $value;
					break;
				default :
					$_7B6C2BDBBD2AF03C64A520410964D2F2 .= " AND " . $key . " LIKE '" . $value . "%'";
					break;
			}
		}
	}
...
if ($start < 0)
	$start = 0;
$SQL = $dg->_821853BAB96B76F492924C9554DBDB09 () . (($_760EB019CDB8F62FF8AA9C0469129954) ? ' WHERE 1=1 ' . $_7B6C2BDBBD2AF03C64A520410964D2F2 : '') . " ORDER BY $sidx $sord";
